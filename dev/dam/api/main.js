...
app.get('/api/download/:file', (req, res) => {
  const ALLOWED = new Set(['dam-agent-linux-amd64', 'dam-agent-linux-arm64', 'dam-agent.exe', 'dam-agent_amd64.deb', 'dam-agent_amd64.rpm', 'dam-agent_arm64.deb', 'dam-agent_arm64.rpm']);
...
// ── Native .xlsx export — a real Excel workbook (Summary + Evidence sheets), no dependency ──
// A .xlsx is a ZIP of OOXML parts. We hand-build the minimal parts + a STORED (uncompressed) zip
// so there is nothing to npm-install on the deploy path.
function _xlCrc32(buf) {
  let crc = 0xFFFFFFFF;
...
const _xlEsc = (s) => String(s).replace(/[<>&'"]/g, (c) => ({ '<': '&lt;', '>': '&gt;', '&': '&amp;', "'": '&apos;', '"': '&quot;' }[c])).replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g, '');
const _xlCol = (n) => { let s = ''; while (n > 0) { const r = (n - 1) % 26; s = String.fromCharCode(65 + r) + s; n = Math.floor((n - 1) / 26); } return s; };
function _xlSheet(rows) {
  let x = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>';
  rows.forEach((row, ri) => {
    x += `<row r="${ri + 1}">`;
    row.forEach((val, ci) => {
      const ref = _xlCol(ci + 1) + (ri + 1);
      if (typeof val === 'number' && isFinite(val)) x += `<c r="${ref}"><v>${val}</v></c>`;
      else x += `<c r="${ref}" t="inlineStr"><is><t xml:space="preserve">${_xlEsc(val == null ? '' : val)}</t></is></c>`;
    });
    x += '</row>';
  });
  return x + '</sheetData></worksheet>';
}
function buildEvidenceXlsx(rec) {
  const snap = rec.result_json || {};
  const rows = Array.isArray(snap.rows) ? snap.rows : [];
  ...
  const summary = [
    ...
    ['Total matched', Number(rec.row_total) || 0], ['Rows in extract', rows.length],
    ['Content SHA-256', rec.content_hash || ''], ['Seal intact', String(sealOk)],
    ['Status', rec.status || 'open'], ['Reviewer', rec.reviewer || ''],
    ['Reviewed at', rec.reviewed_at ? new Date(rec.reviewed_at).toISOString() : ''], ['Reviewer note', rec.reviewer_note || ''],
  ];
  const cols = ['ts', 'principal', 'database_name', 'object', 'operation', 'rows', 'client_ip', 'tags', 'sql_preview'];
  const headers = ['Time', 'Principal', 'Database', 'Object', 'Operation', 'Rows', 'Client IP', 'Tags', 'Statement'];
  const evidence = [headers, ...rows.map((r) => cols.map((c) => {
    if (c === 'rows') { const n = parseInt(r[c]); return isFinite(n) ? n : (r[c] || ''); }
  ...
  const files = [
    ...
    { name: 'xl/workbook.xml', data: '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Summary" sheetId="1" r:id="rId1"/><sheet name="Evidence" sheetId="2" r:id="rId2"/></sheets></workbook>' },
    { name: 'xl/_rels/workbook.xml.rels', data: '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet2.xml"/></Relationships>' },
    { name: 'xl/worksheets/sheet1.xml', data: _xlSheet(summary) },
    { name: 'xl/worksheets/sheet2.xml', data: _xlSheet(evidence) },
  ];
  return _zipStore(files);
}
app.get('/api/compliance/evidence/:id/xlsx', authRequired, async (req, res) => {
  try {
    ...
    const buf = buildEvidenceXlsx(rec);
    await writeAudit({ tenantId: req.user.tenantId, actorId: req.user.userId, actorEmail: req.user.email, action: 'compliance.evidence.export.xlsx', resourceType: 'evidence', resourceId: rec.id, details: {} });
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="evidence-${rec.catalog_id}-${String(rec.id).slice(0, 8)}.xlsx"`);
    res.send(buf);
  } catch (e) { console.error('[Compliance] evidence xlsx failed:', e.message); res.status(500).json({ error: 'Excel export failed' }); }
});
