...
- Enabled per-agent with `CLASSIFY=true` + `DB_USER` / `DB_PASSWORD`; re-scans every `CLASSIFY_INTERVAL_MIN` minutes (default 30), ~10s after each restart, and **on demand** via the Classification page's **Run Scan** button (the agent polls a per-tenant, token-authed `scan-pending` every ~12s). Same outbound-only path as capture — the control plane never dials into the DB network.
